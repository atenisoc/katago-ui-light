...
(function () {
  ...
  const persona = { name: "Kaya", style: { tone: "calm-analytical-with-human-touch", targetLen: [60, 110] } };
  ...
  // Kaya: 含みのある言い回し（断定を避ける）
  if (shape > 0 && volatility > 0) {
    refs.push("流れは少しこちらに寄ってきたように感じる。ただ霧が濃く、見通しは楽ではない。");
  }
  if (shape < 0 && volatility < 0) {
    refs.push("形勢は静かに離れていったように見える。一方で局面は落ち着き始めている。");
  }
  if (shape > 0 && volatility <= 0) {
    refs.push("確かな追い風を感じる。盤は静まりつつあって、道筋も少し見えやすい。");
  }
  if (shape < 0 && volatility > 0) {
    refs.push("状況は悪化しているようだし、不安定さも増している。ここは慎重に受けたい。");
  }
  ...
  async function describe({ bestMove, katago, moves }) {
    try {
      ...
      const skeleton = {
        persona: { role: "kaya-female-calm-analytical-subtle" },
        context: {
          phase,
      ...
      const payload = {
        ...
        referenceDirectives: {
          ...
          keep_persona: "kaya-female-calm-analytical-subtle",
          sentences: "1-2",
 ...
 setPersona(p) {
   if (!p) return;
   const { name, ...rest } = p;
   Object.assign(persona, rest); // name は "Kaya" のまま固定
 },
