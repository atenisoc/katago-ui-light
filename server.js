...
// 先頭（プロセス情報を一元管理）
const procs = {};        // {easy:{proc, rl, waiters, pid, startedAt}, ...}
const engineMeta = {};   // {easy:{version, modelName, backend, katago}, ...}
...
// ====== KataGo プロセス管理 ======
/**
 * procs[name] = { proc, rl, waiters, pid, startedAt }
 */
...
function spawnEngine(name) {
  ...
  proc.on("exit", (code, signal) => {
    ...
    try { delete procs[name]; } catch {}
    setTimeout(() => {
      try { spawnEngine(name); } catch (e) { console.error(`[${name}] respawn failed:`, e); }
    }, 1500);
  });

  procs[name] = { proc, rl, waiters, pid: proc.pid, startedAt: Date.now() };
