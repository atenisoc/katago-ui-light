...
const ENGINES = {
  ...
  normal: {
    ...
    // ★ b10 を置いたらこちらに差し替える（現状 b6 を指している場合は弱い）
    model: "C:\\tools\\katago\\engines\\normal_b10\\weights\\kata1-b6c96-s175395328-d26788732.txt.gz",
    cfg: "C:\\tools\\katago\\engines\\normal_b10\\analysis.cfg",
  },
  hard: { // 評価用
    exe: "C:\\tools\\katago\\engines\\hard_b18\\katago.exe",
    model: "C:\\tools\\katago\\engines\\hard_b18\\weights\\kata1-b10c128-s1141046784-d204142634.txt.gz",
    // 可能なら b18/b28 に差し替え推奨（例：kata1-b18c384nbt.bin.gz / kata1-b28c512nbt-....txt.gz）
...
app.post("/api/analyze", async (req, res) => {
  try {
    ...
    if (!payload.rules) payload.rules = "japanese";
    if (typeof payload.komi !== "number") payload.komi = 6.5;
    // payload.includeOwnership = false; // 対局では省略でOK
    ...
    res.json({ ok: true, bestMove, katago: katagoResp });
  } catch (e) {
