...
const openai = process.env.OPENAI_API_KEY
  ? new OpenAI({
      ...
      project: process.env.OPENAI_PROJECT,   // ← これ必須
    })
  : null;
...
app.post("/api/comment", async (req, res) => {
  try {
    ...
    // aiComment.js 側で“設計”は完了しているので、ここは薄いリレー
    const prompt = [
      SYSTEM_PROMPT,
      "",
      makeUserPrompt(skeleton, banPhrases, lo, hi)
    ].join("\n");
    ...
    const rsp = await openai.responses.create({
      ...
      temperature: 0.9,
      top_p: 0.95
      // v5 Responses では presence_penalty / frequency_penalty は未対応
      // 必要なら後で別の方法で制御。まずは通す。
    });

    // 出力候補の収集（Responses API）
    const cands = [];
    for (const item of rsp.output || []) {
      for (const c of item.content || []) {
        if (c.type === "output_text" && c.text) {
          cands.push(
            c.text.trim().replace(/\s+/g, " ").replace(/\u3002+$/g, "。")
          );
        }
      }
    }
    const uniq = [...new Set(cands)].filter(Boolean);
    if (!uniq.length) return res.status(502).json({ error: "no_candidates" });
    return res.json({ text: pickLeastSimilar(uniq, banPhrases) });
