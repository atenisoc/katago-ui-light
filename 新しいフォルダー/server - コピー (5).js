...
function ensureNormalized(payload) {
  const size = payload.boardXSize || 19;
  if (!Array.isArray(payload.moves)) payload.moves = [];
  const mv = payload.moves;
  const isPair = Array.isArray(mv[0]) && mv[0].length >= 2 && typeof mv[0][0] === 'string';
  if (!isPair) {
    payload.moves = normalizeMovesToKataGo(mv, size);
  } else {
    payload.moves = mv.map(p => [String(p[0]).toLowerCase(), String(p[1])]);
  }
  payload.__movesNormalized = true;
  // Validate minimal shape
  if (payload.moves.length > 0) {
    const ok = Array.isArray(payload.moves[0]) && (payload.moves[0][0] === 'b' || payload.moves[0][0] === 'w') && typeof payload.moves[0][1] === 'string';
    if (!ok) throw new Error("moves normalization failed");
  }
  return payload;
}
/* ==============================
...
function startKataGo() {
  ...
  kg.stderr.on("data", (d) => {
    ...
    process.stderr.write("[KataGo STDERR] " + text);
    // Try to capture JSON error lines with an id and reject the matching waiter
    try {
      const m = text.match(/\{\"error\":.*?\}/g);
      if (m) {
        for (const seg of m) {
          try {
            const obj = JSON.parse(seg);
            if (obj && obj.id && obj.error) {
    const infl = inFlight.get(obj.id);
    if (infl) { infl.resolve(obj); inFlight.delete(obj.id); }
            }
          } catch {}
        }
      }
...
function katagoRequest(payload) {
  try { ensureNormalized(payload); } catch(e) { return Promise.reject(e); }
  return new Promise((resolve, reject) => {
    if (!kg) startKataGo();
    const id = "req_" + (++reqCounter);
    payload.id = id;
    inFlight.set(id, { resolve, reject });
    console.log("[Analyze(katagoRequest)] -> KataGo:", JSON.stringify(payload));
...
app.post("/api/analyze", async (req, res) => {
  try {
    ...
    const { moves = [], rules = "japanese", komi = 6.5, maxTime, maxVisits } = req.body || {};
    ...
    const payload = {
      ...
      boardXSize: 19,
      boardYSize: 19,
      ...(Number.isFinite(maxVisits) ? { maxVisits: Math.max(1, Math.floor(maxVisits)) } : {}),
      moves: normalizeMovesToKataGo(moves, 19),            // normalized to [['b','D4'],['w','Q16'],...]
      // 必要なら includeOwnership / includePolicy をここで true/false
      };

    // 書く（1行JSON + 改行）→ 待つ
    const p = new Promise((resolve, reject) => waiters.set(id, { resolve, reject }));
    ensureNormalized(payload);
    ...
    // 最小の返却（安定版同等）
    const moveInfos = Array.isArray(result?.moveInfos) ? result.moveInfos : [];
    // Choose best by (1) explicit order if present, else (2) highest visits, else (3) first element
    let best = null;
    if (moveInfos.length > 0) {
      if (moveInfos[0] && typeof moveInfos[0].order === "number") {
        best = moveInfos.reduce((a,b)=> (a && a.order <= b.order ? a : b));
      } else if (moveInfos[0] && typeof moveInfos[0].visits === "number") {
        best = moveInfos.reduce((a,b)=> (a && a.visits >= b.visits ? a : b));
      } else {
        best = moveInfos[0];
      }
    }
    let bestMove = best && best.move ? best.move : null;
    // Extra fallbacks: try principal variation or rootInfo if available
    if (!bestMove && result?.rootInfo) {
      const pv = Array.isArray(result.rootInfo.principalVariation) ? result.rootInfo.principalVariation : null;
      if (pv && pv.length > 0) bestMove = pv[0];
      if (!bestMove && typeof result.rootInfo.move === "string") bestMove = result.rootInfo.move;
    }
...
// --- Ensure moves are normalized to KataGo format ---
function ensureNormalized(payload) {
  ...
  if (!isPair) {
    ...
    // lowercase color + ensure GTP vertex string
    payload.moves = mv.map(p => {
      const c = String(p[0]).toLowerCase();
      const v = String(p[1]);
      return [c, v];
    });
  }
  ...
  // Validate
  if (payload.moves.length > 0) {
    const ok = Array.isArray(payload.moves[0]) &&
              (payload.moves[0][0] === 'b' || payload.moves[0][0] === 'w') &&
              typeof payload.moves[0][1] === 'string';
    if (!ok) throw new Error("moves normalization failed");
  }
  return payload;
}
